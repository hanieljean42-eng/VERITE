...
import { Gender } from "@/lib/analyzer";

interface Props {
  participants: string[];
  onComplete: (youName: string, otherName: string, otherGender: Gender) => void;
}
...
export default function SetupScreen({ participants, onComplete }: Props) {
  ...
  const canProceed = youName && otherName && otherGender;
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-5 py-8">
      ...
      <div className="w-full max-w-sm space-y-5">
        ...
        {/* Bouton continuer */}
        <button
          onClick={() => canProceed && onComplete(youName, otherName, otherGender!)}
          disabled={!canProceed}
